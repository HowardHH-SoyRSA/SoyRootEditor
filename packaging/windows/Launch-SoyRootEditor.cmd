...
powershell -NoProfile -Command "try { $r = Invoke-WebRequest -UseBasicParsing -TimeoutSec 2 -SessionVariable viewerSession 'http://127.0.0.1:8765/'; if ($r.Content -notmatch 'SoyRoot Studio') { exit 1 }; $health = Invoke-RestMethod -TimeoutSec 2 -WebSession $viewerSession 'http://127.0.0.1:8765/api/health'; if ($health.viewer_id -and $health.viewer_compatibility -eq 'final-support-v1') { exit 0 }; exit 2 } catch { exit 1 }" >nul 2>&1
if errorlevel 2 (
  echo An incompatible SoyRootEditor service is already using port 8765.
  echo Close the older viewer, then launch this shortcut again.
  exit /b 2
)
if not errorlevel 1 (
  if "%~1"=="" (
